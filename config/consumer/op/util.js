...
async function operationWithRetry(
  ...
  try {
    ...
    // Catch error from promise - not how I would do it normally, but allows re use of existing code.
...
async function insertIntoSpecificGraphs(lib, statementsWithGraphs) {
  for (let graph in statementsWithGraphs) {
    console.log(
      `Inserting ${statementsWithGraphs[graph].length} statements into ${graph} graph`,
    );
    await batchedDbUpdate(
      lib.muAuthSudo.updateSudo,
      graph,
...
async function deleteFromSpecificGraphs(lib, statementsWithGraphs) {
  for (let graph in statementsWithGraphs) {
    console.log(
      `Deleting ${statementsWithGraphs[graph].length} statements from ${graph} graph`,
    );
    await batchedDbUpdate(
      lib.muAuthSudo.updateSudo,
      graph,
...
async function transformLandingZoneGraph(fetch, endpoint, mapping = "main") {
  ...
  const response = await fetch(
    `http://reasoner/reason/op2clb/${mapping}?data=${encodeURIComponent(`${endpoint}?default-graph-uri=&query=CONSTRUCT+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D+WHERE+%7B%0D%0A+GRAPH+<${LANDING_ZONE_GRAPH}>+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D%0D%0A%7D&should-sponge=&format=text%2Fplain&timeout=0&run=+Run+Query`)}`,
  );
